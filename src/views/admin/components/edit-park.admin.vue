.<template>
  <div class="edit-park">
        ...
        <md-button class="md-raised md-accent" style="background-color:#069a9a" @click="open()">Editar</md-button>

       <md-dialog :md-active.sync="showDialog">

            <md-dialog-title>EDITAR PARQUE</md-dialog-title>
            ...
            <md-dialog-actions>
                ...
                <md-button class="md-primary" @click="edit()">Guardar</md-button>
            </md-dialog-actions>
...
export default class AdminAuthView extends Vue {
    ...
    @Prop() public park!: IFireBasePark;
    ...
    open(){
        ...
        this.logo =this.park.logo || 'https://www.flaticon.com/svg/static/icons/svg/896/896008.svg';
        this.capacity = this.park.capacity;

        this.park.weekdays.monday.forEach(schedule => {
            const hour = schedule.hour;
            this.monday[hour] = true;
        });
        this.park.weekdays.tuesday.forEach(schedule => {
            const hour = schedule.hour;
            this.tuesday[hour] = true;
        });
        this.park.weekdays.wednesday.forEach(schedule => {
            const hour = schedule.hour;
            this.wednesday[hour] = true;
        });
        this.park.weekdays.thursday.forEach(schedule => {
            const hour = schedule.hour;
            this.thursday[hour] = true;
        });
        this.park.weekdays.friday.forEach(schedule => {
            const hour = schedule.hour;
            this.friday[hour] = true;
        });
        this.park.weekdays.saturday.forEach(schedule => {
            const hour = schedule.hour;
            this.saturday[hour] = true;
        });
        this.park.weekdays.sunday.forEach(schedule => {
            const hour = schedule.hour;
            this.sunday[hour] = true;
        });
   ...
   public getSchedules(day){
        ...
        Object.keys(this[day]).forEach(hourKey => {
            ...
            const types = [
                Object.keys(hoursPublic).includes(hourKey) && this[day][hourKey] ? 'PUBLIC' : '' ,
                Object.keys(hoursSchool).includes(hourKey) && this[day][hourKey] ? 'SCHOOL': ''
            ].filter(type => type && type != '')
            if(isEnabled){
                schedules.push({
                    hour: hourKey,
                    aviableFor: types as Array<'PUBLIC' | 'SCHOOL'>,
                    capacity: {
